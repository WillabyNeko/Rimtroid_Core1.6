    ...
    public class Ability_AbsorbEnergy : RT_Core.Ability_Base
    {
        ...
        public override bool CanApplyOn(LocalTargetInfo target)
        {
            Predicate<LocalTargetInfo> validator = delegate (LocalTargetInfo targetInfo)
            {
                ...
                {
                    return !Utils.blackListRaces.Contains(corpse.InnerPawn.def)
                ...
                else if (target.Thing is Pawn victim2 && (pawn.Faction is null && !victim2.IsAnyMetroid() || pawn.Faction == Faction.OfPlayer) && !Utils.blackListRaces.Contains(victim2.def) 
        ...
        {
            if (target.Thing is Corpse corpse && !Utils.blackListRaces.Contains(corpse.InnerPawn.def)
                && corpse.GetRotStage() == RotStage.Fresh && corpse.Age < GenDate.TicksPerDay * 3 && pawn.CanReserve(corpse))
            {
